...
Below is a basic example which shows you how to perform cross validation with a sequence of kernels by using ```gpr_cv```:
...
If you are interested in performing Gaussian process regression by using multiple kernels, below is an example of how to use ```gpr_seq_kernels``` to do the prediction. Note that kernels passed into this function need to be using the Euclidean distance between two data points as parameters.
```{r example1 gpr_seq_kernels}
library(standardGP)
library(matrixcalc)
# intent inputs
n <- 100 # number of inputs
...
There are some predefined functions in the package that you can choose from by using ```pick_kernel```:
```{r example1 pick_kernel}
...
## References
[1] Rasmussen, Carl Edward, and Christopher K. I. Williams. Gaussian Processes for Machine Learning.
MIT Press, 2005.
